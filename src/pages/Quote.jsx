...
export default class Quote extends React.Component {
  constructor(props) {
    ...
    const quotes = data.quotes.map(quote => mergeObjects(quote, { image: requireImage(quote.scene, quote.character) }));
    // set initial state
    this.state = { 
  ...
  switchQuote = (next = true) => {
    ...
    // define next/prev quote id
    const nextId = this.state.id === this.state.quotes.length - 1 ? 0 : this.state.id + 1;
    const prevId = this.state.id === 0 ? this.state.quotes.length - 1 : this.state.id - 1;
    // show next/prev quote
    setTimeout(() => this.setState({ id: next ? nextId : prevId, show: true }), 300);
  }
  ...
  render() {
    ...
    return (
      <div className={`character-bg ${this.state.show ? 'show' : 'hide'}`} 
        style={{ backgroundImage: `url(${this.quote.image})` }} onKeyPress={this.handleKeyPress}>
